...
class ValorantDataCollector:
    ...
    def get_player_stats(self, url: str) -> Dict:
        ...
        # Append the timespan filter for 'all' days
        ...
        team_tag = soup.find('div', class_='wf-title-med')
        team = team_tag.text.strip() if team_tag else 'No Team'
...
def export_player_stats_to_csv(player_data, filename):
    """Exports player stats to a CSV file with a row for each agent."""
    with open(filename, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([
            'Name', 'Team', 'Agent', 'Usage', 'Rounds Played', 'Rating', 'ACS', 'K:D',
...
def collect_and_save_data(player_names: List[str]):
    ...
    for player_name in player_names:
        ...
        if player_url:
            ...
            if player_stats:
                export_player_stats_to_csv(player_stats, f"{player_name}_stats.csv")
